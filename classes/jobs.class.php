...
class WPEC_Jobs extends WPEC_ecommerce_feeder {
	...
	function saveJob( $data = false ) {
		...
		if ( ! $this->isGood( $data['direction'] ) && $_REQUEST['direction'] ) {
			$data['direction'] = $_REQUEST['direction'];
		}
	...
	function getScheudledJobs( $options = false ) {
		...
		if ( $this->isGood( $options['id'] ) && $this->isGood( $this->savedJobs[ $options['id'] ] ) ) {
			...
			if ( $this->isGood( $options['direction'] ) && $this->isGood( $this->savedJobs ) && in_array( $options['direction'],
					array( 'import', 'export' ) )
			) {
	...
	function runJobs( $options = false ) {
		...
		$this->logger->info( "WPEC_Jobs::runJobs(" . print_r( $options,
				true ) . ") running jobs " . print_r( $jobs, true ) );
	...
	function getCount( &$instructions = false ) {
		...
		if ( isset( $direction ) && $direction == 'import' && isset( $type ) && array_key_exists( $type,
				$scripts[ $direction ] )
		) {
			$valid = apply_filters( 'ecommerce_feeder_validateJob_' . $type, $instructions );
			if ( $valid ) {
				$count = apply_filters( 'ecommerce_feeder_import_get_count_' . $type,
					$object,
					$instructions );
			}
		} elseif ( isset( $direction ) && $direction == 'export' && isset( $type ) && array_key_exists( $type,
				$scripts[ $direction ] )
		) {
	...
	function runDataTypeImport( $dataType, $dataSet ) {
		...
		switch ( $dataType ) {
			...
			case 'products':
				...
				break;
			default:
				$this->logger->error( "Failed to Run Job, incorrect object.:" . print_r( $instructions,
						true ) );
				break;
		}
	}
	...
	function runDataTypeExport( $dataType ) {
		...
		switch ( $dataType ) {
			...
			default:
				$this->logger->error( "Failed to Run Job, incorrect object.:" . print_r( $instructions,
						true ) );
				$_SESSION['error_msg'] = __( "Failed to Run Job, incorrect object.:",
						'ecommerce_feeder' ) . print_r( $instructions, true );
				break;
		}
		return ( $dataSets );
	}
